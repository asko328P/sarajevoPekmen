...
const MovePlayer = (entities: any, infoObj: any) => {
  for (const [entityKey, entityValue] of Object.entries(entities)) {
    ...
    if (
      ...
      const desiredMovementAngle = entityValue.desiredMovementAngle;
      const position = entityValue.position;
      const nextPosition = entityValue.nextPosition;
      const previousPosition = entityValue.previousPosition;
      const playerSpeed = entityValue.playerSpeed;
      ...
      if (angleDifference(desiredMovementAngle, currentMovementAngle) > 90 && currentMovementAngle !== 0) {
        ...
        entityValue.previousPosition = nextPosition;
      }
